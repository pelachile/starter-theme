...
function css() {
	...
	$color_link   = get_theme_mod( CHILD_TEXT_DOMAIN . '_link_color', \CCWP\Functions\customizer_get_default_link_color() );
	$color_accent = get_theme_mod( CHILD_TEXT_DOMAIN . '_accent_color', \CCWP\Functions\customizer_get_default_accent_color() );
	$logo         = wp_get_attachment_image_src( get_theme_mod( 'custom_logo' ), 'full' );

	if ( $logo ) {
		$logo_height           = absint( $logo[2] );
		$logo_max_width        = get_theme_mod( CHILD_TEXT_DOMAIN . '_logo_width', 350 );
	...
	$css .= ( \CCWP\Functions\customizer_get_default_link_color() !== $color_link ) ? sprintf(
		'
	...
	$css .= ( \CCWP\Functions\customizer_get_default_accent_color() !== $color_accent ) ? sprintf(
		'
		...
		', $color_accent, \CCWP\Functions\color_contrast( $color_accent )
	) : '';
